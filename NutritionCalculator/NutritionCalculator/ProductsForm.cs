            ...
            using (Form overlayForm = new Form())
            {
                ...
                using (AddProductForm addProductForm = new AddProductForm())
                {
                    ...
                    if (addProductForm.DialogResult == DialogResult.OK)
                    {
                        ...
                        {
                            case "Основні страви":
                                filePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"food.json");
                                break;
                            case "Перші страви":
                                filePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"firstDish.json");
                                break;
                            case "Солодкі страви":
                                filePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"sweetFood.json");
                                break;
                            case "Снеки":
                                filePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"pocketFood.json");
                                break;
                            case "Напої":
                                filePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"drink.json");
                                break;
                        }
                ...
                using (DeleteProductForm deleteProductForm = new DeleteProductForm(GetProductNames(), GetCategories()))
                {
                    ...
                    if (deleteProductForm.DialogResult == DialogResult.OK)
                    {
                        ...
                        if (category == "Основні страви")
                        {
                            ...
                            SaveProductsToFile(foodItems, Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"food.json"));
                        }
                        else if (category == "Перші страви")
                        {
                            RemoveProduct(firstDishes, productName);
                            SaveProductsToFile(firstDishes, Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"firstDish.json"));
                        }
                        else if (category == "Солодкі страви")
                        {
                            RemoveProduct(sweetFoods, productName);
                            SaveProductsToFile(sweetFoods, Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"sweetFood.json"));
                        }
                        else if (category == "Снеки")
                        {
                            RemoveProduct(pocketFoods, productName);
                            SaveProductsToFile(pocketFoods, Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"pocketFood.json"));
                        }
                        else if (category == "Напої")
                        {
                            RemoveProduct(drinks, productName);
                            SaveProductsToFile(drinks, Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "source\\repos\\NutritionCalculator\\NutritionCalculator\\foodCategories", $"drink.json"));
                        }
                        InitializeDataGridView();
                    }
                }
